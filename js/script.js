...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Storage
  // ─────────────────────────────────────────────────────────────
  // Open state: session only (won’t auto-open after tab/browser fully closed)
  const STORAGE_KEY_OPEN = 'helpWidgetOpen_session';
  // Position: localStorage (keeps after refresh + browser reopen)
  const STORAGE_KEY_POS = 'helpPopupPosition';

  // If you had old keys before, you can clean them once:
  // localStorage.removeItem('helpWidgetOpen');

  // ─────────────────────────────────────────────────────────────
  // Elements (NEW structure)
  // ─────────────────────────────────────────────────────────────
  const btn = document.getElementById('helpBtn'); // .help-widget-btn
  const popup = document.getElementById('helpPopup'); // .help-popup
  const header = document.getElementById('helpPopupHeader'); // .help-popup__header
  const closeBtn = document.getElementById('helpClose'); // .help-popup__close

  const form = popup?.querySelector('.help-popup__form');
  const messages = document.getElementById('messagesContainer');

  if (!btn || !popup || !header || !closeBtn) {
    console.warn('Help widget elements missing');
    return;
  }

  // ─────────────────────────────────────────────────────────────
  // 1) Open / Close + save state (sessionStorage)
  // ─────────────────────────────────────────────────────────────
  function isOpen() {
  ...
  // Restore open state ONLY on refresh (F5/Ctrl+R).
  // On normal open/reopen -> always closed.
  function isReloadNavigation() {
    const nav = performance.getEntriesByType?.('navigation')?.[0];
    if (nav && typeof nav.type === 'string') return nav.type === 'reload';
    return performance?.navigation?.type === 1; // fallback
  }
  ...
  // ─────────────────────────────────────────────────────────────
  // 2) Default popup position (FIRST TIME)
  //    opens bottom-right (independent from button)
  // ─────────────────────────────────────────────────────────────
  function setDefaultPopupPosition() {
    popup.style.left = 'auto';
    popup.style.right = '58px';
  ...
  // ─────────────────────────────────────────────────────────────
  // 3) Restore popup position (localStorage)
  // ─────────────────────────────────────────────────────────────
  function restorePosition() {
    try {
      ...
      console.warn('Invalid saved position', err);
    }
  ...
  // ─────────────────────────────────────────────────────────────
  // 4) Draggable popup (mouse + touch) via Pointer Events
  // ─────────────────────────────────────────────────────────────
  ...
  function startDrag(e) {
    ...
    // capture pointer (important for touch)
    try {
      header.setPointerCapture(pointerId);
    } catch (_) {}

    // Make sure popup has left/top before dragging (if it was bottom-based)
  ...
  function onPointerMove(e) {
    ...
    const pad = 8;
    const maxLeft = window.innerWidth - popup.offsetWidth - pad;
    const maxTop = window.innerHeight - popup.offsetHeight - pad;
  ...
  if (form && messages) {
    form.addEventListener('submit', function (e) {
      ...
      const input = form.querySelector('.help-popup__input');
      if (!input) return;
      ...
      userMsg.className = 'help-popup__msg help-popup__msg--user';
      userMsg.innerHTML = `<div class="help-popup__msg-bubble">${text}</div>`;
      messages.appendChild(userMsg);

      setTimeout(() => {
        const reply = document.createElement('div');
        reply.className = 'help-popup__msg help-popup__msg--support';
        reply.innerHTML =
          `<div class="help-popup__msg-bubble">Спасибо за ваш вопрос! Мы скоро ответим.</div>`;
